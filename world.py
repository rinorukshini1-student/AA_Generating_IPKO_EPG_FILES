...
def day_range(offset_days=0):
    day = datetime.now(timezone(timedelta(hours=1))) + timedelta(days=offset_days)  
...
def epg_to_smart_json(epg_data):
    ...
    for i, (channel, ch_data) in enumerate(epg_data.items()):
        ...
        for pid, show in enumerate(ch_data.get("shows", []), start=1):
            ...
            base = show["show_title"]
            title_counter[base] += 1

            if title_counter[base] == 1:
                prog_id = base
            else:
                prog_id = f"{base}_{pid}"
...
def get_genre_links():
    ...
    if not genres:
        print(f" No genres found on {START_URL}")
    return genres
...
def get_genre_day_links(genre_url):
    try:
        ...
        print(f" Failed to fetch genre page {genre_url}: {e}")
        return []

    soup = BeautifulSoup(html, "html.parser")
    day_links = [BASE_URL + a["href"] for a in soup.select(".card-group a[href*='/epg/']")]
    if not day_links:
        print(f" No day links found on {genre_url}")
    return day_links
...
def get_programs_from_day(day_url, genre):
    try:
        ...
        print(f" Failed to fetch {day_url}: {e}")
        return []
...
while True:
    ...
    for day_offset in range(choiceDays):
        ...
        if not daily_epg:
            print(f" No programs fetched for {date_str}")
            continue
